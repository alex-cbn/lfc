			...
			if(ts->exists($1) == 1)
		  	{
		  		if(ts->getType($1)==$4->getType())
				{
					...
					printf("WIN? %s\n", $4->correct_order.c_str());
					printf("Rezidual depth%d\n", depth);
				}
				else
				{
...
E_BFIS:	
	E_BFIS TOK_PLUS E_BFIS
	{
		...
		else
		{
			...
			printf("?+?\tLEFT:%s[%d]\tRIGHT:%s[%d]\n", $1->correct_order.c_str(), $1->is_in_eax, $3->correct_order.c_str(), $3->is_in_eax);
			if($3->is_in_eax)
			{
				$$->correct_order = $3->correct_order + " + " + $1->correct_order;
				printf(">>%s\t\n", $$->correct_order.c_str());
			}
			else
			{
				$$->correct_order = $1->correct_order + " + " + $3->correct_order;
				printf(">>%s\t\n", $$->correct_order.c_str());
			}
			if(!$3->is_in_eax && !$3->is_in_eax)
			{
				$$->correct_order = std::to_string(*(int*)$1->getValue()) + " + " + std::to_string(*(int*)$3->getValue());
				printf(">>%s\t\n", $$->correct_order.c_str());
			}
			$$->is_in_eax=1;
		}
    ...
    E_BFIS TOK_MINUS E_BFIS
	{
		...
		else
		{
			...
			if($3->is_in_eax == 0 && !$3->is_in_eax == 0)
			{
				$$->correct_order = std::to_string(*(int*)$1->getValue()) + " - " + std::to_string(*(int*)$3->getValue());
				printf(">>%s\t\n", $$->correct_order.c_str());
			}
